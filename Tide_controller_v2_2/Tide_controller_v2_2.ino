/* Tide_controller_v2.2
 ...
 limit switches. Designed to work with daughterboard rev 7. 
...
//#define FULLSTEP  // uncomment to use full step mode
...
/*  
 ...
 tide height in units of feet. The final conversion depends on whether
 you're using full-step mode or microstepping mode with at 200-step/turn
 motor.
 */
#ifdef FULLSTEP
 ...
 */
  const float stepConv = 0.00004165;   // Value for 10 tpi lead screw, full-step
#else  // microstep mode
  ...
  */
  const float stepConv = 0.000002604;   // Value for 10 tpi lead screw, 1/16th step
#endif

//------------------------------------------------
// Set the delay between individual motor steps. This 
// determines the speed of the motor.
// Units are microseconds (not milliseconds)
#ifdef FULLSTEP
  const int stepDelay = 5000; // define delay between steps of the motor. 
#else // microstep mode
  const int stepDelay = 200; // define delay between steps of the motor. 
  // 200 seems to be a good balance between power and smoothness.
#endif
//*******************************
...
{  
  ...
  pinMode(rxPin, OUTPUT);  // re-use rxPin to dis/enable microstepping mode
  
  digitalWrite(stepperDir, LOW);
  digitalWrite(stepperStep, LOW);
  digitalWrite(stepperEnable, HIGH); // turns off motor power when high
  
  // Using pre-processor directive to choose whether to use full step
  // or 1/16th microstep mode by changing pin 4 (rxPin) state.  
  #ifdef FULLSTEP
    digitalWrite(rxPin, LOW); // Pull rxPin to ground for full-step mode
    Serial.println("Full step mode");
  #else
    digitalWrite(rxPin, HIGH); // Pull rxPin high for 1/16th step mode
    Serial.println("Microstepping mode");
  #endif
